...
<div class="ui inverted segment">
  <div class="ui inverted form">
	...
	<div class="field" id="botname">
        <label>Name</label>
        <div class="ui large labeled input"><input type="text" value="${params.get('name') || "Tate Brothers"}" placeholder="Nickname" maxlength="18" spellcheck="false" data-ms-editor="true"></div>
      </div>
      <div class="field">
        <label>Room</label>
...
<div class="ui fluid selection profil bot-image dropdown" tabaindex="0">
  ...
  <div data-value="0" class="item active selected">
        <img class="ui avatar image" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTGZZI2WMTSE-Rm2YYKFlxmQqwrpWvPpAyyFw&usqp=CAU"1" class="item">
...
async function addBot(botAmount) {
  ...
  for (let i = 0; i < t; i++) {
    ...
    if (params.get('private-mode') === "true") {
      ...
      try {
        ...
        if (people) {
          ...
          if (regex.test(username)) {
            document.querySelector('#botname div input').value = "Tate Brothers";
            username = "";
          }
        }
      ...
      if (regex.test(name)) {
        document.querySelector('#botname div input').value = "Top G";
        name = "Top G";
      }
    ...
    socket.addEventListener('message', (event) => {
      ...
      switch (data[0]) {
        case 5: {
          ...
          if (params.get('private-mode') !== "true") { socket.send(`42[11,"${playerId}"," ı am owner top g tate brothers "]`); }
...
document.querySelector("#send").addEventListener("click", function () {
  ...
  if (regex.test(messagetext)) {
    document.querySelector('#messagetext input').value = "Tate Brothers";
    messagetext = "Tate Brothers";
  }
